...
function buildProblems(facts: HeuristicFacts): Problem[] {
  ...
  if (facts.responsibilitiesCount > facts.achievementsCount * 2) {
    ...
    add(
      ...
      "Резюме читается как должностная инструкция, а не как подтверждение ценности.",
  ...
  if (facts.effectiveCount >= 4) {
    add(
      ...
      "Повтор эффективности без метрик выглядит как попытка звучать серьёзно без фактов.",
...
function buildTheatreFindings(facts: HeuristicFacts): TheatreFinding[] {
  const findings: TheatreFinding[] = [
    ...
    },
    {
      id: "t3",
      stage: "seniority",
      message: `Заявлено ${facts.claimedLevel}, по содержанию ближе к ${facts.inferredLevel}.`,
    },
  ];
...
> = {
  tamara: (p) => {
    const byTitle: Record<string, string> = {
      ...
      "Корпоративные клише":
        "Клише мы читаем как отсутствие конкретики. Факты — пожалуйста, прилагательные — нет.",
      "Инфляция «эффективности»":
        "Эффективность без цифры в корпорации называется «мнение».",
      "Внутреннее противоречие":
        "В одном файле две несовместимые версии событий. Какую утверждаем?",
      "Мало конкретики":
        "Материала недостаточно, чтобы согласовать уровень. Добавьте конкретику.",
    };
    return {
  ...
  gleb: (p) => {
    const byTitle: Record<string, string> = {
      ...
      "Мало конкретики":
        "Выборка слишком мала для вывода об уровне. Расширьте фактическую базу.",
    };
    return {
      roast: byTitle[p.title] ?? p.roast,
      comment:
        "Документ страдает не от недостатка слов, а от недостатка фактической опоры.",
    };
  },
  vadik: (p) => {
    const byTitle: Record<string, string> = {
